...
public class QLSnake : MonoBehaviour
{
    ...
    float epilson = 1.0f;
    float epilson_min = 0.005f;
    float epilson_decay = 0.99993f;
    ...
    void Update()
    {
        ...
        else if (dead == true)
        {
            ...
            }else
            {
                if(iteration % 100 == 0)
                {
                    string file_name = "qtable" + iteration.ToString() + ".txt";
                    Save(file_name);
                    RewardRate(iteration, reward);
                }
    ...
    public int GetAction(string state)
    {
        ...
        if (num < epilson || !qtable.ContainsKey(state))
        {
    ...
    public void UpdateTable(int action, string state, string nextstate)
    {
        NewRow(state);
        NewRow(nextstate);
        float newValue = reward + discount * Mathf.Max(qtable[nextstate][0], qtable[nextstate][1], qtable[nextstate][2], qtable[nextstate][3]);
        qtable[state][action] = ((1 - learning_rate) * qtable[state][action]) + (learning_rate * newValue);
    }
    ...
    public void RewardRate(int iteration, float reward)
    {
        File.AppendAllText("Reward.txt", string.Format("{0}, {1}", iteration.ToString(), reward.ToString() + System.Environment.NewLine));
    }
    ...
    public void Save(string file_name)
    {
        if (qtable != null)
        {
            ...
            foreach (var entry in qtable)
                content += entry.Key + "," + entry.Value[0]+  "," + entry.Value[1] + "," + entry.Value[2] + "," + entry.Value[3] + "\n";
    ...
    {
        SceneManager.LoadScene("QLearningTrained");
    }
